...
export class ControllerGenerator extends BaseGenerator {
  async generate(ctx: GeneratorContext): Promise<GeneratedFile[]> {
    ...
    const destPath = path.join(outputDir, `${ctx.context.nameKebab}.controller.ts`);
    const file = await this.renderAndWrite('__NAME_KEBAB__.controller.ts.tpl', destPath, ctx);
    return [file];
  }
}
